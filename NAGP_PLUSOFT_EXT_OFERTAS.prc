CREATE OR REPLACE PROCEDURE CONSINCO.NAGP_PLUSOFT_EXT_OFERTAS IS
...
BEGIN
    ...
    v_file := UTL_FILE.fopen('/u02/app_acfs/arquivos/plusoft', 'Ext_Plusoft_Ofertas_'||v_Periodo||'.csv', 'w', 32767);
    ...
    -- Executa a query e escreve os resultados
      ...
      FOR vda IN (SELECT *                                           
                    FROM NAGV_PLUSOFT_OFERTAS 
                   WHERE 1=1)

      LOOP

        v_line := vda.NRO_EMPRESA||';'||vda.CODOFERTA||';'||vda.IDPRODUTO||';'||vda.DESCRICAOPRODUTO||';'||vda.DATAINICIO||';'||vda.DATAFIM||';'||vda.MODALIDADE||';'||vda.QTDMAXIMACOMPRA||';'||vda.PERCENTUALDESCONTO||';'||vda.VLRDE||';'||vda.VLRPOR||';'||vda.QTDLEVE||';'||vda.QTDPAGUE||';'||vda.QTDAPARTIRDE||';'||vda.VITRINE||';'||vda.DATAINICIOF||';'||vda.DATAFIMF;
